...
def search(query: str, limit: int = 5, client: httpx.Client | None = None,
    ...
    Взрослое здесь по умолчанию видно, в отличие от дайджеста: человек
    спросил про конкретный тайтл, и молча отдать ему урезанный список —
    значит соврать. Дайджест же приходит всей беседе без спроса.
    """
    return _fetch(
        "/api/animes",
